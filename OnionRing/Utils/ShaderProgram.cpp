...
std::string ShaderProgram::spStringShaderTypes[ShaderProgram::COUNT_SHADER_TYPE] = 
{
	"Vertex Shader",
	"Geometry Shader",
...
ShaderProgram::ShaderProgram(): pProgram(0), pInited(false) {
...
void ShaderProgram::setProgramOutput(int index, std::string name) {
	glBindFragDataLocation(pProgram, index, name.c_str());
}


GLint ShaderProgram::getProgramOutput(std::string name) {
	return glGetFragDataLocation(pProgram, name.c_str());
}


void ShaderProgram::setVertexAttribName(ShaderProgram::AttribType at, std::string name) {
...
GLuint ShaderProgram::getShaderIndex(ShaderProgram::ShaderType aType) {
	return pShader[aType];
}


void ShaderProgram::setBlock(std::string name, void *value) {
...
void ShaderProgram::setBlockUniform(std::string blockName, 
...
void ShaderProgram::setBlockUniformArrayElement(std::string blockName, 
								std::string uniformName,
								int arrayIndex, 
								void * value) {
...
void ShaderProgram::setUniform(std::string name, void *value) {
...
std::string ShaderProgram::getShaderInfoLog(ShaderProgram::ShaderType st) {
...
std::string ShaderProgram::getProgramInfoLog() {
	...
	if (pProgram) {
		...
		if (infologLength > 0)
		{
			...
			pResult = infoLog;
            if(pResult == "Validation Failed: Link error:\n  Program is not successfully linked.\n")
            {
                pResult = "";
                free(infoLog);
                return pResult;
...
bool ShaderProgram::isProgramValid()
{
    if(getProgramInfoLog().size() == 0) return true;
	GLint b = GL_FALSE;
...
bool ShaderProgram::isShaderCompiled(ShaderProgram::ShaderType aType) {
...
void ShaderProgram::addUniform(std::string name, GLenum type, unsigned int size) {
...
int ShaderProgram::typeSize(int type) {
